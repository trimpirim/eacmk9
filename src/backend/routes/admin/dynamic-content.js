...
const routes = (loggedMiddleware) => {
        ...
        .get(loggedMiddleware, (req, res) => {
            DynamicContent.find({}).sort([['createdAt', 'descending']]).exec((err, dynamicContents) => {
                ...
                const existingContents = {
                    ...
                    ourDogsInformation: dynamicContents.filter((item) => {
                        return item.identifier == DynamicContent.TYPES.OUR_DOGS_INFORMATION
                    }).length > 0
                }

                res.status(200);
                res.render('admin/dynamic-content/list', {
    ...
    router.route('/create')
        ...
        .get(loggedMiddleware, (req, res) => {
            res.status(200);
            res.render('admin/dynamic-content/create', {
                dynamicContent: res.locals.dynamicContentObject,
        ...
        .post([loggedMiddleware], (req, res, next) => {
            ...
            if (!req.form.isValid) {
                return res.render('admin/dynamic-content/create', {
                    dynamicContent: res.locals.dynamicContentObject,
                    form: req.form,
                    which: req.query.which
                })
    ...
    router.route('/edit/:id')
        ...
        .post([loggedMiddleware], (req, res, next) => {
            ...
            if (!req.form.isValid) {
                return res.render('admin/dynamic-content/edit', {
                    dynamicContent: res.locals.dynamicContentObject,
                    form: req.form
                })
            }
