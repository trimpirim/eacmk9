            ...
            if (err) {
                ...
                Award.find({}).exec((err, awards) => {
                    if (err) {
                        ...
                        let disciplines = [...new Set(Dog.DISCIPLINES.map(discipline => {
                            return discipline.group
                        }))].map(group => {
                            const items = Dog.DISCIPLINES.filter(discipline => {
                                return discipline.group === group
                            }).map(discipline => {
                                return {title: discipline.title, value: discipline.id, group: discipline.group}
                            })
                            return {title: group, group: true, items: items}
                        })

                        let genders = [{
                            title: i18n.__('admin.dogs.gender.female'),
                            value: Dog.GENDERS.FEMALE
                        }, {title: i18n.__('admin.dogs.gender.male'), value: Dog.GENDERS.MALE}]
...
api.route('/dogs')
    .get((req, res) => {
        ...
        const filter = Filter.parseFilter({
            ...
            awards: {
                criteria: {
                    $in: [handledForm.title, handledForm.certificate].filter(object => {
                        return !!object
                    })
                },
